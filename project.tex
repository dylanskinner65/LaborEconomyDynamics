...
\begin{itemize}
    ...
    \item $\epsilon$: Efficiency of white-collar jobs in utilizing blue-collar jobs
...
In our first run of the model, we used parameters $\rho = 7$, $a = 5$, $\mu = 1$, $\varepsilon = .2$, $k = 3$, $e = 1$, $C = 1.5$. As we see, the model oscilates
...
\begin{figure}[h]
    ...
    \caption{With the parameters $\rho = 7$, $a = 5$, $\mu = 1$, $\varepsilon = .2$, $k = 3$, $e = 1$, $C = 1.5$, we can see that the model oscilates
...
Consider two different sets of initial conditions, namely $rho = 7$, $a = 5$, $\mu = 2$, $\varepsilon = .2$, $k = 3$, $e = 1$, $C = 1.5$ and 
$rho = 7$, $a = 5$, $\mu = 1$, $\varepsilon = .2$, $k = 3$, $e = 1$, $C = 1.5$. Note that the only difference between these two initial conditions is
...
\begin{figure}[h]
    ...
    \caption{The left graph corresponds to $\mu = 2$, while the right graph corresponds to $\varepsilon = .2$, others parameters are kept the same.}
...
in figure~\ref{fig:results_wb_3} that a change of $1$ in $\mu$ causes the populations to entirely flip. Additionally, changing epsilon from $.2$ to $1$ causes 
